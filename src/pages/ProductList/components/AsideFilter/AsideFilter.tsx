...
export default function AsideFilter({ queryConfig, categories, pathname = path.home }: Props) {
  ...
  return (
    <div className='py-6'>
      <Link
        to={pathname}
        className={classNames('flex items-center font-bold text-xl', {
          'text-orange': !category
        })}
      >
        <svg viewBox='0 0 12 10' className='mr-4 h-6 w-4 fill-current'>
      ...
      <ul>
        {categories.map((categoryItem) => {
          ...
          return (
            <li className='py-3 pl-3' key={categoryItem.id}>
              <Link
                to={{
                  pathname: pathname,
                  search: createSearchParams({
                ...
                className={classNames('relative px-3 text-lg', {
      ...
      <Link to={pathname} className='mt-6 flex items-center font-bold uppercase text-xl'>
        <svg
          enableBackground='new 0 0 15 15'
          viewBox='0 0 15 15'
          x={0}
          y={0}
          className='mr-4 h-6 w-4 fill-current stroke-current'
        >
          <g>
            <polyline
              fill='none'
      ...
      <div className='my-6'>
        ...
        <form className='mt-4' onSubmit={onSubmit}>
          <div className='flex items-start gap-3'>
              ...
              render={({ field }) => {
                return (
                  <InputNumber
                    ...
                    className='grow'
                    placeholder='₫ TỪ'
                    classNameInput='p-3 w-full outline-none border border-gray-300 focus:border-gray-500 rounded-lg focus:shadow-sm text-lg'
            ...
            />
            <div className='mt-3 shrink-0 text-xl'>-</div>
            <Controller
              control={control}
              name='price_max'
              render={({ field }) => {
